...
{
    /// <summary>
    /// LinkedIn implementation of IPostPlugin interface
    /// </summary>
    [Export(typeof(IPlugin))]
    public class PNVKontakte : IPostPlugin
    {
        ...
        public string AdditionalInfo => "Uses VKontakte.NET library by Andrey Gruber (http://pnotes.sourceforge.net/)";

        public void Init(IPluginsHost host)
        {
            _Host = host;
            _MenuPostPartial = new ToolStripMenuItem(Name, Properties.Resources.vk, menuClick);
            _MenuPostFull = new ToolStripMenuItem(Name, Properties.Resources.vk, menuClick);
            _MenuGetPartial = new ToolStripMenuItem(Name, Properties.Resources.vk, menuClick);
            _MenuGetFull = new ToolStripMenuItem(Name, Properties.Resources.vk, menuClick);
        }
        ...
        private void menuClick(object sender, EventArgs e)
        {
            ...
            if (item.Equals(_MenuPostPartial))
            {
                ...
                if (edit != null)
                {
                    var result = VKPost.Post(edit.SelectedText, _Host.ActiveCulture);
            ...
            else if (item.Equals(_MenuPostFull))
            {
                ...
                if (edit != null)
                {
                    var result = VKPost.Post(edit.Text, _Host.ActiveCulture);
                    PostPerformed?.Invoke(this, new PostPerformedEventArgs(result));
                }
            }
            else if (item.Equals(_MenuGetPartial))
            {
                var details = VKPost.Get(_Host.LimitToGet, _Host.ActiveCulture);
