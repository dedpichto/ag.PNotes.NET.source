...
namespace pnftp
{
    ...
    public class PNFTP : ISyncEnhPlugin
    {
        ...
        {
            _Host = host;
            _MenuSync = new ToolStripMenuItem(Name, Properties.Resources.ftp, menuClick);
        }
        ...
        public string AdditionalInfo => "Uses C# FTP library by Scott Meyer (http://ftplib.codeplex.com/)";

        public SyncResult Synchronize()
        {
            var ftps = new FTPSync();
            var result = ftps.Synchronize(_Host.SyncParameters, _Host.ActiveCulture);
        ...
        {
            var ftps = new FTPSync();
            ftps.InnerSyncComplete += gds_InnerSyncComplete;
            ftps.SynchronizeInBackground(_Host.SyncParameters);
        }

        private void gds_InnerSyncComplete(object sender, SyncCompleteEventArgs e)
        {
            if (sender is FTPSync ftps)
                ftps.InnerSyncComplete -= gds_InnerSyncComplete;
