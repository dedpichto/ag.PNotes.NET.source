...
namespace PNotes.NET
{
    internal class PNData
    {
        ...
        internal static string SettingsConnectionString { get; set; } = "";

        internal static string ConnectionString { get; set; } = "";

        internal static string ContactsConnectionString { get; set; } = "";

        internal static bool FirstRun { get; private set; }
        ...
        internal static void SaveReportFields(ReportBitField fields, string dates)
        {
            try
            {
                using (var oData = new SQLiteDataObject(SettingsConnectionString))
                {
                    var sqlQuery = "UPDATE CONFIG SET REPORT_FIELDS = " + fields.Value.ToString(PNRuntimes.Instance.CultureInvariant);
                    oData.Execute(sqlQuery);
                    sqlQuery = "UPDATE CONFIG SET REPORT_DATES = '" + dates + "'";
                    oData.Execute(sqlQuery);
                }
            }
            catch (Exception ex)
        ...
        internal static void SaveExitFlag(int flag)
        {
            try
            {
                using (var oData = new SQLiteDataObject(SettingsConnectionString))
                {
                    var sqlQuery = "UPDATE CONFIG SET EXIT_FLAG = " + flag.ToString(PNRuntimes.Instance.CultureInvariant);
                    oData.Execute(sqlQuery);
                }
            }
            catch (Exception ex)
        ...
        internal static void LoadHiddenMenus()
        {
            try
            {
                PNCollections.Instance.HiddenMenus.Clear();
                using (var oData = new SQLiteDataObject(ConnectionString))
                {
                    using (var t = oData.FillDataTable("SELECT * FROM HIDDEN_MENUS"))
                    {
                        ...
                        {
                            PNCollections.Instance.HiddenMenus.Add(new PNHiddenMenu(Convert.ToString(r["MENU_NAME"]),
        ...
        /// <summary>
        /// Since version 3.8 some tables should be transferred from notes.db3 to settings.db3
        /// </summary>
        private static void transferTablesFromNotesToSettings()
        {
            try
            {
                var list = new List<string>();
                using (var oDataContacts = new SQLiteDataObject(ContactsConnectionString))
                {
                    using (var oDataNotes = new SQLiteDataObject(ConnectionString))
                    {
                        //CONTACTS table
                        //create table
                        var query =
                            $"CREATE TABLE IF NOT EXISTS [{PNStrings.T_CONTACTS}] ([ID] INT PRIMARY KEY ON CONFLICT REPLACE NOT NULL UNIQUE ON CONFLICT REPLACE, [GROUP_ID] INT NOT NULL, [CONTACT_NAME] TEXT NOT NULL UNIQUE ON CONFLICT REPLACE, [COMP_NAME] TEXT NOT NULL, [IP_ADDRESS] TEXT NOT NULL, [USE_COMP_NAME] BOOLEAN NOT NULL)";
                        oDataContacts.Execute(query);
                        query = $"SELECT count(*) FROM sqlite_master WHERE type='table' AND name='{PNStrings.T_CONTACTS}';";
                        var count = oDataNotes.GetScalar(query);
                        if (count != null && !IsDBNull(count) && Convert.ToInt32(count) != 0)
                        {
                            //CONTACTS table should be transferred
                            using (var t = oDataNotes.FillDataTable($"SELECT * FROM {PNStrings.T_CONTACTS}"))
                            {
                                //transfer data
                                list.Clear();
                                foreach (DataRow r in t.Rows)
                                {
                                    list.Add(
                                        $"INSERT INTO {PNStrings.T_CONTACTS} VALUES({r["ID"]}, {r["GROUP_ID"]}, '{r["CONTACT_NAME"]}', '{r["COMP_NAME"]}', '{r["IP_ADDRESS"]}', {Convert.ToInt32(r["USE_COMP_NAME"])})");
                                }
                                var transSet = oDataContacts.BeginTransaction();
                                var transNotes = oDataNotes.BeginTransaction();
                                try
                                {

                                    foreach (var s in list)
                                    {
                                        oDataContacts.ExecuteInTransaction(s);
                                    }

                                    oDataNotes.ExecuteInTransaction($"DROP TABLE {PNStrings.T_CONTACTS}");
                                    oDataNotes.CommitTransaction();
                                    transNotes = false;
                                    oDataContacts.CommitTransaction();
                                    transSet = false;
                                }
                                catch (Exception ex)
                                {
                                    if (transSet)
                                        oDataContacts.RollbackTransaction();
                                    if (transNotes)
                                        oDataNotes.RollbackTransaction();
                                    PNStatic.LogException(ex);
                                    return;
                                }
                            }
                        ...
                        using (
                            var t =
                                oDataContacts.FillDataTable(
                                    $"SELECT ID, GROUP_ID, CONTACT_NAME, COMP_NAME, IP_ADDRESS, USE_COMP_NAME FROM {PNStrings.T_CONTACTS}")
                        )
                        {
                        ...
                        //CONTACTS_GROUPS table
                        query =
                            $"CREATE TABLE IF NOT EXISTS [{PNStrings.T_CONTACT_GROUPS}] ([ID] INT PRIMARY KEY ON CONFLICT REPLACE NOT NULL UNIQUE ON CONFLICT REPLACE, [GROUP_NAME] TEXT NOT NULL UNIQUE ON CONFLICT REPLACE)";
                        oDataContacts.Execute(query);
                        query = $"SELECT count(*) FROM sqlite_master WHERE type='table' AND name='{PNStrings.T_CONTACT_GROUPS}';";
                        count = oDataNotes.GetScalar(query);
                        if (count != null && !IsDBNull(count) && Convert.ToInt32(count) != 0)
                        {
                            //CONTACT_GROUPS table should be transferred
                            using (var t = oDataNotes.FillDataTable($"SELECT * FROM {PNStrings.T_CONTACT_GROUPS}"))
                            {
                                //transfer data
                                list.Clear();
                                foreach (DataRow r in t.Rows)
                                {
                                    list.Add($"INSERT INTO {PNStrings.T_CONTACT_GROUPS} VALUES({r["ID"]}, '{r["GROUP_NAME"]}')");
                                }
                            }
                            var transSet = oDataContacts.BeginTransaction();
                            var transNotes = oDataNotes.BeginTransaction();
                            try
                            {
                                ...
                                oDataNotes.ExecuteInTransaction($"DROP TABLE {PNStrings.T_CONTACT_GROUPS}");
                                oDataNotes.CommitTransaction();
                                transNotes = false;
                                oDataContacts.CommitTransaction();
                                transSet = false;
                        ...
                        using (var t = oDataContacts.FillDataTable($"SELECT * FROM {PNStrings.T_CONTACT_GROUPS}"))
                        {
                            foreach (DataRow r in t.Rows)
                            {
                        ...
                        ////mail contacts
                        //query =
                        //    $"CREATE TABLE IF NOT EXISTS [{PNStrings.T_MAIL_CONTACTS}] ([ID] INT UNIQUE ON CONFLICT REPLACE, [DISPLAY_NAME] TEXT, [ADDRESS] TEXT )";
                        //oDataContacts.Execute(query);
                        //query = $"SELECT count(*) FROM sqlite_master WHERE type='table' AND name='{PNStrings.T_MAIL_CONTACTS}';";
                        //count = oDataNotes.GetScalar(query);
                        //if (count != null && !IsDBNull(count) && Convert.ToInt32(count) != 0)
                        //{
                        //    //MAIL_CONTACTS table should be transferred
                        //    using (var t = oDataNotes.FillDataTable($"SELECT * FROM {PNStrings.T_MAIL_CONTACTS}"))
                        //    {
                        //        //transfer data
                        //        list.Clear();
                        //        foreach (DataRow r in t.Rows)
                        //        {
                        //            list.Add($"INSERT INTO {PNStrings.T_MAIL_CONTACTS} VALUES({r["ID"]}, '{r["DISPLAY_NAME"]}', '{r["ADDRESS"]}')");
                        //        }
                        //    }
                        //    var transSet = oDataContacts.BeginTransaction();
                        //    var transNotes = oDataNotes.BeginTransaction();
                        ...
                        //        oDataNotes.ExecuteInTransaction($"DROP TABLE {PNStrings.T_MAIL_CONTACTS}");
                        //        oDataNotes.CommitTransaction();
                        //        transNotes = false;
                        //        oDataContacts.CommitTransaction();
                        //        transSet = false;
                        ...
                        //    }
                        //}
                        //using (var t = oDataContacts.FillDataTable($"SELECT * FROM {PNStrings.T_MAIL_CONTACTS}"))
                        //{
                        //    foreach (DataRow r in t.Rows)
                        //    {
        ...
        private static void updateContactsWithoutIp()
        {
        ...
        private static void createPaths()
        {
            try
            {
                ...
                {
                    PNRuntimes.Instance.SpTextProvider.SplashText = PNLang.Instance.GetMessageText("create_database",
                        "Creating database");
                    FirstRun = true;
                }
                ...
                if (!File.Exists(Path.Combine(PNPaths.Instance.DictDir, "dictionaries.xml")))
                {
                    var streamResourceInfo =
                        Application.GetResourceStream(new Uri(@"xml/dictionaries.xml", UriKind.Relative));
                ...
                PNRuntimes.Instance.Dictionaries = XDocument.Load(Path.Combine(PNPaths.Instance.DictDir, "dictionaries.xml"));
            }
            catch (Exception ex)
            {
                PNStatic.LogException(ex);
        ...
        internal static void LoadDBSettings()
        {
            try
            {
                ...
                PNRuntimes.Instance.Settings = new PNSettings();

                ConnectionString = SQLiteDataObject.CheckAndCreateDatabase(PNPaths.Instance.DBPath);
                SettingsConnectionString = SQLiteDataObject.CheckAndCreateDatabase(PNPaths.Instance.SettingsDBPath);
                ContactsConnectionString = SQLiteDataObject.CheckAndCreateDatabase(PNPaths.Instance.ContactsDBPath);

                transferTablesFromNotesToSettings();

                using (var oData = new SQLiteDataObject(SettingsConnectionString))
                {
                    string sqlQuery;
                    //config
                    var pnc = PNRuntimes.Instance.Settings.Config;
                    if (!oData.TableExists("CONFIG"))
                    {
                        sqlQuery = "CREATE TABLE [CONFIG] ([LAST_PAGE] TEXT, [EXIT_FLAG] INT, [CP_LAST_GROUP] INT, [SKINNABLE] BOOLEAN, [CP_PVW_COLOR] TEXT, [CP_USE_CUST_PVW_COLOR] BOOLEAN, [CP_SIZE] TEXT, [CP_LOCATION] TEXT, [CONTROLS_STYLE] TEXT, [CP_PVW_RIGHT] BOOLEAN, [UI_FONT] TEXT, [PROGRAM_VERSION] TEXT, [CP_PVW_SHOW] BOOLEAN, [CP_GROUPS_SHOW] BOOLEAN, [NOTES_WITH_SHORTCUTS] TEXT, [SEARCH_NOTES_SETT] TEXT, [REPORT_FIELDS] INT, [REPORT_DATES] TEXT, [CP_IGNORE_CASE] BOOLEAN, [LAST_LOCAL_SYNC] TEXT, [LAST_PLUGIN_SYNC] TEXT)";
                        oData.Execute(sqlQuery);
                        sqlQuery =
                            "INSERT INTO CONFIG VALUES(NULL, -1, NULL, NULL, NULL, NULL, '1000,600', NULL, NULL, NULL, NULL, '" +
                            v.ToString(3) + "', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL)";
                        oData.Execute(sqlQuery);
                    ...
                    else
                    {
                        using (var t = oData.GetSchema("Columns"))
                        {
                            ...
                            if (rows.Length == 0)
                            {
                                ...
                                PNRuntimes.Instance.SpTextProvider.SplashText = PNLang.Instance.GetMessageText("back_prev",
                            ...
                            rows = t.Select("COLUMN_NAME = 'REPORT_FIELDS' AND TABLE_NAME = 'CONFIG'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE CONFIG ADD COLUMN [REPORT_FIELDS] INT";
                                oData.Execute(sqlQuery);
                            }
                            rows = t.Select("COLUMN_NAME = 'REPORT_DATES' AND TABLE_NAME = 'CONFIG'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE CONFIG ADD COLUMN [REPORT_DATES] TEXT";
                                oData.Execute(sqlQuery);
                            }
                            rows = t.Select("COLUMN_NAME = 'CP_IGNORE_CASE' AND TABLE_NAME = 'CONFIG'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE CONFIG ADD COLUMN [CP_IGNORE_CASE] BOOLEAN";
                                oData.Execute(sqlQuery);
                                sqlQuery = "UPDATE CONFIG SET CP_IGNORE_CASE = 1";
                                oData.Execute(sqlQuery);
                            }
                            rows = t.Select("COLUMN_NAME = 'LAST_LOCAL_SYNC' AND TABLE_NAME = 'CONFIG'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE CONFIG ADD COLUMN [LAST_LOCAL_SYNC] TEXT";
                                oData.Execute(sqlQuery);
                            }
                            rows = t.Select("COLUMN_NAME = 'LAST_PLUGIN_SYNC' AND TABLE_NAME = 'CONFIG'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE CONFIG ADD COLUMN [LAST_PLUGIN_SYNC] TEXT";
                                oData.Execute(sqlQuery);
                            }
                        }
                        //store version
                        ...
                        if (PNSingleton.Instance.PlatformChanged)
                        {
                            ...
                            {
                                obj = drawingConverter.ConvertFromString(null, PNRuntimes.Instance.CultureInvariant, (string)obj);
                                if (obj != null)
                                {
                        ...
                        using (var t = oData.FillDataTable(sqlQuery))
                        {
                            if (t.Rows.Count > 0)
                            {
                                ...
                                {
                                    var convertFromString = mediaConverter.ConvertFromString(null, PNRuntimes.Instance.CultureInvariant, (string)r["CP_PVW_COLOR"]);
                                    if (
                                ...
                                if (!IsDBNull(r["CP_SIZE"]))
                                {
                                    ...
                                    if (!str.Contains('-'))
                                    {
                                        ...
                                        var convertFromString = sc.ConvertFromString(null, PNRuntimes.Instance.CultureInvariant,
                                            str);
                                ...
                                {
                                    var convertFromString = pc.ConvertFromString(null, PNRuntimes.Instance.CultureInvariant, (string)r["CP_LOCATION"]);
                                    if (
                                        convertFromString != null)
                                ...
                                pnc.ReportSettings = !IsDBNull(r["REPORT_FIELDS"])
                                    ? new ReportBitField(Convert.ToInt32(r["REPORT_FIELDS"]))
                                    : new ReportBitField();
                                if (!IsDBNull(r["REPORT_DATES"]))
                                {
                                    pnc.ReportDates = Convert.ToString(r["REPORT_DATES"]);
                                }
                                ...
                                if (!IsDBNull(r["SEARCH_NOTES_SETT"]))
                                {
                                    ...
                                    pnc.SearchNotesSettings.WholeWord = Convert.ToBoolean(arr[0]);
                                ...
                                if (!IsDBNull(r["CP_IGNORE_CASE"]))
                                {
                                    pnc.CPIgnoreCase = Convert.ToBoolean(r["CP_IGNORE_CASE"]);
                                }
                                if (!r.IsNull("LAST_LOCAL_SYNC"))
                                {
                                    pnc.LastLocalSync = Convert.ToDateTime(r["LAST_LOCAL_SYNC"]);
                                }
                                if (!r.IsNull("LAST_PLUGIN_SYNC"))
                                {
                                    pnc.LastPluginSync = Convert.ToDateTime(r["LAST_PLUGIN_SYNC"]);
                                }
                            }
                            else
                            {
                    ...
                    //general setting
                    var pngeneral = PNRuntimes.Instance.Settings.GeneralSettings;
                    ...
                    else
                    {
                        using (var t = oData.GetSchema("Columns"))
                        {
                            ...
                            rows = t.Select("COLUMN_NAME = 'SHOW_SCROLLBAR' AND TABLE_NAME = 'GENERAL_SETTINGS'");
                            if (rows.Length == 1)
                            {
                                if (Convert.ToString(rows[0]["DATA_TYPE"]).ToUpper() == "BOOLEAN")
                                {
                                    //change column type to INT
                                    recreateGeneralSettingsTable();
                                }
                            }
                        }
                            ...
                            if (t.Rows.Count > 0)
                            {
                                ...
                                if (!IsDBNull(r["TIME_FORMAT"]))
                                {
                                    ...
                                    Thread.CurrentThread.CurrentCulture.DateTimeFormat.ShortTimePattern =
                                        PNStatic.StripDateTimeFormat(pngeneral.TimeFormat, 's');
                                }
                                ...
                                if (!IsDBNull(r["SPELL_COLOR"]))
                                {
                                    pngeneral.SpellColor = (System.Drawing.Color)
                                            drawingConverter.ConvertFromString(null, PNRuntimes.Instance.CultureInvariant,
                    ...
                    PNRuntimes.Instance.SpTextProvider.SplashText = PNLang.Instance.GetMessageText("load_program_settings", "Loading program settings");

                    //schedule
                    if (!oData.TableExists("SCHEDULE"))
                    {
                        sqlQuery = "CREATE TABLE [SCHEDULE] ([SOUND] TEXT, [DATE_FORMAT] TEXT, [TIME_FORMAT] TEXT, [VOICE] TEXT, [ALLOW_SOUND] BOOLEAN, [TRACK_OVERDUE] BOOLEAN, [VISUAL_NOTIFY] BOOLEAN, [CENTER_SCREEN] BOOLEAN, [VOICE_VOLUME] INT, [VOICE_SPEED] INT, [VOICE_PITCH] INT, [FIRST_DAY_OF_WEEK] INT, [FIRST_DAY_TYPE] INT, [STOP_AFTER] INT)";
                        oData.Execute(sqlQuery);
                        sqlQuery = "INSERT INTO SCHEDULE VALUES(NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL)";
                        oData.Execute(sqlQuery);
                    }
                    else
                    {
                        using (var t = oData.GetSchema("Columns"))
                        {
                            var rows = t.Select("COLUMN_NAME = 'FIRST_DAY_TYPE' AND TABLE_NAME = 'SCHEDULE'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE SCHEDULE ADD COLUMN [FIRST_DAY_TYPE] INT";
                                oData.Execute(sqlQuery);
                            }
                            rows = t.Select("COLUMN_NAME = 'FIRST_DAY_OF_WEEK' AND TABLE_NAME = 'SCHEDULE'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE SCHEDULE ADD COLUMN [FIRST_DAY_OF_WEEK] INT";
                                oData.Execute(sqlQuery);
                            }
                            rows = t.Select("COLUMN_NAME = 'STOP_AFTER' AND TABLE_NAME = 'SCHEDULE'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE SCHEDULE ADD COLUMN [STOP_AFTER] INT";
                                oData.Execute(sqlQuery);
                            }
                        }
                        ...
                        using (var t = oData.FillDataTable(sqlQuery))
                        {
                            ...
                            {
                                var r = t.Rows[0];
                                var pnsc = PNRuntimes.Instance.Settings.Schedule;
                                if (!IsDBNull(r["SOUND"]))
                                {
                                ...
                                if (!r.IsNull("FIRST_DAY_OF_WEEK"))
                                {
                                    pnsc.FirstDayOfWeek = (DayOfWeek)r["FIRST_DAY_OF_WEEK"];
                                }
                                if (!r.IsNull("FIRST_DAY_TYPE"))
                                {
                                    pnsc.FirstDayOfWeekType = (FirstDayOfWeekType)r["FIRST_DAY_TYPE"];
                                }
                                if (!IsDBNull(r["STOP_AFTER"]))
                                {
                                    pnsc.StopAfter = (int)r["STOP_AFTER"];
                                }
                            }
                            else
                            {
                    ...
                    else
                    {
                        ...
                        using (var t = oData.FillDataTable(sqlQuery))
                        {
                            ...
                            {
                                var r = t.Rows[0];
                                var pnb = PNRuntimes.Instance.Settings.Behavior;
                    ...
                    {
                        sqlQuery = "CREATE TABLE [PROTECTION] ([STORE_AS_ENCRYPTED] BOOLEAN, [HIDE_TRAY_ICON] BOOLEAN, [BACKUP_BEFORE_SAVING] BOOLEAN, [SILENT_FULL_BACKUP] BOOLEAN, [BACKUP_DEEPNESS] INT, [DO_NOT_SHOW_CONTENT] BOOLEAN, [INCLUDE_BIN_IN_SYNC] BOOLEAN, [PASSWORD_STRING] TEXT, [FULL_BACKUP_DAYS] TEXT, [FULL_BACKUP_TIME] TEXT, [FULL_BACKUP_DATE] TEXT, [PROMPT_PASSWORD] BOOLEAN, [LOCAL_SYNC_FILES] TEXT, [LOCAL_SYNC_PERIOD] INT, [LOCAL_SYNC_TIME] TEXT, [LOCAL_SYNC_TYPE] INT, [AUTO_SYNC_RESTART] BOOLEAN)";
                        oData.Execute(sqlQuery);
                        sqlQuery = "INSERT INTO PROTECTION VALUES(NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL)";
                        oData.Execute(sqlQuery);
                    }
                    else
                    {
                        using (var t = oData.GetSchema("Columns"))
                        {
                            ...
                            rows = t.Select("COLUMN_NAME = 'LOCAL_SYNC_FILES' AND TABLE_NAME = 'PROTECTION'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE PROTECTION ADD COLUMN [LOCAL_SYNC_FILES] TEXT";
                                oData.Execute(sqlQuery);
                            }
                            rows = t.Select("COLUMN_NAME = 'LOCAL_SYNC_PERIOD' AND TABLE_NAME = 'PROTECTION'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE PROTECTION ADD COLUMN [LOCAL_SYNC_PERIOD] INT";
                                oData.Execute(sqlQuery);
                            }
                            rows = t.Select("COLUMN_NAME = 'LOCAL_SYNC_TIME' AND TABLE_NAME = 'PROTECTION'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE PROTECTION ADD COLUMN [LOCAL_SYNC_TIME] TEXT";
                                oData.Execute(sqlQuery);
                            }
                            rows = t.Select("COLUMN_NAME = 'LOCAL_SYNC_TYPE' AND TABLE_NAME = 'PROTECTION'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE PROTECTION ADD COLUMN [LOCAL_SYNC_TYPE] INT";
                                oData.Execute(sqlQuery);
                            }
                            rows = t.Select("COLUMN_NAME = 'AUTO_SYNC_RESTART' AND TABLE_NAME = 'PROTECTION'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE PROTECTION ADD COLUMN [AUTO_SYNC_RESTART] BOOLEAN";
                                oData.Execute(sqlQuery);
                            }
                        }
                        ...
                        using (var t = oData.FillDataTable(sqlQuery))
                        {
                            ...
                            {
                                var r = t.Rows[0];
                                var pnp = PNRuntimes.Instance.Settings.Protection;
                                ...
                                {
                                    pnp.FullBackupTime = DateTime.Parse((string)r["FULL_BACKUP_TIME"],
                                        PNRuntimes.Instance.CultureInvariant);
                                }
                                //else
                                //{
                                //    pnp.FullBackupTime = DateTime.Now;
                                //}
                                if (!IsDBNull(r["FULL_BACKUP_DATE"]))
                                {
                                    pnp.FullBackupDate = DateTime.Parse((string)r["FULL_BACKUP_DATE"],
                                        PNRuntimes.Instance.CultureInvariant);
                                }
                                ...
                                if (!IsDBNull(r["LOCAL_SYNC_FILES"]))
                                {
                                    pnp.LocalSyncFilesLocation = (string)r["LOCAL_SYNC_FILES"];
                                }
                                if (!r.IsNull("LOCAL_SYNC_PERIOD"))
                                {
                                    pnp.LocalSyncPeriod = (int)r["LOCAL_SYNC_PERIOD"];
                                }
                                if (!r.IsNull("LOCAL_SYNC_TIME"))
                                {
                                    pnp.LocalSyncTime = TimeSpan.Parse((string)r["LOCAL_SYNC_TIME"]);
                                }
                                if (!r.IsNull("LOCAL_SYNC_TYPE"))
                                {
                                    pnp.LocalSyncType = (SilentSyncType)r["LOCAL_SYNC_TYPE"];
                                }
                                if (!r.IsNull("AUTO_SYNC_RESTART"))
                                {
                                    pnp.AutomaticSyncRestart = (bool)r["AUTO_SYNC_RESTART"];
                                }
                            }
                            else
                            {
                    ...
                    else
                    {
                        ...
                        using (var t = oData.FillDataTable(sqlQuery))
                        {
                            ...
                            {
                                var r = t.Rows[0];
                                var pndr = PNRuntimes.Instance.Settings.Diary;
                    ...
                    else
                    {
                        using (var t = oData.GetSchema("Columns"))
                        {
                            ...
                            rows = t.Select("COLUMN_NAME = 'STORE_ON_SERVER' AND TABLE_NAME = 'NETWORK'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE NETWORK ADD COLUMN [STORE_ON_SERVER] BOOLEAN";
                                oData.Execute(sqlQuery);
                            }
                            rows = t.Select("COLUMN_NAME = 'SERVER_IP' AND TABLE_NAME = 'NETWORK'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE NETWORK ADD COLUMN [SERVER_IP] TEXT";
                                oData.Execute(sqlQuery);
                            }
                            rows = t.Select("COLUMN_NAME = 'SERVER_PORT' AND TABLE_NAME = 'NETWORK'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE NETWORK ADD COLUMN [SERVER_PORT] INT";
                                oData.Execute(sqlQuery);
                            }
                            rows = t.Select("COLUMN_NAME = 'SEND_TIMEOUT' AND TABLE_NAME = 'NETWORK'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE NETWORK ADD COLUMN [SEND_TIMEOUT] INT";
                                oData.Execute(sqlQuery);
                            }
                            rows = t.Select("COLUMN_NAME = 'SILENT_SYNC_PERIOD' AND TABLE_NAME = 'NETWORK'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE NETWORK ADD COLUMN [SILENT_SYNC_PERIOD] INT";
                                oData.Execute(sqlQuery);
                            }
                            rows = t.Select("COLUMN_NAME = 'SILENT_SYNC_TIME' AND TABLE_NAME = 'NETWORK'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE NETWORK ADD COLUMN [SILENT_SYNC_TIME] TEXT";
                                oData.Execute(sqlQuery);
                            }
                            rows = t.Select("COLUMN_NAME = 'SILENT_SYNC_TYPE' AND TABLE_NAME = 'NETWORK'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE NETWORK ADD COLUMN [SILENT_SYNC_TYPE] INT";
                                oData.Execute(sqlQuery);
                            }
                            rows = t.Select("COLUMN_NAME = 'AUTO_SYNC_RESTART' AND TABLE_NAME = 'NETWORK'");
                            if (rows.Length == 0)
                            {
                                sqlQuery = "ALTER TABLE NETWORK ADD COLUMN [AUTO_SYNC_RESTART] BOOLEAN";
                                oData.Execute(sqlQuery);
                            }
                        }
                        ...
                        using (var t = oData.FillDataTable(sqlQuery))
                        {
                            ...
                            {
                                var r = t.Rows[0];
                                var pnw = PNRuntimes.Instance.Settings.Network;
                                if (!IsDBNull(r["INCLUDE_BIN_IN_SYNC"]))
                                {
                                ...
                                if (!IsDBNull(r["STORE_ON_SERVER"]))
                                {
                                    pnw.StoreOnServer = (bool)r["STORE_ON_SERVER"];
                                }
                                if (!IsDBNull(r["SERVER_IP"]))
                                {
                                    pnw.ServerIp = (string)r["SERVER_IP"];
                                }
                                if (!IsDBNull(r["SERVER_PORT"]))
                                {
                                    pnw.ServerPort = (int)r["SERVER_PORT"];
                                }
                                if (!IsDBNull(r["SEND_TIMEOUT"]))
                                {
                                    pnw.SendTimeout = (int)r["SEND_TIMEOUT"];
                                }
                                if (!r.IsNull("SILENT_SYNC_PERIOD"))
                                {
                                    pnw.SilentSyncPeriod = (int)r["SILENT_SYNC_PERIOD"];
                                }
                                if (!r.IsNull("SILENT_SYNC_TIME"))
                                {
                                    pnw.SilentSyncTime = TimeSpan.Parse((string)r["SILENT_SYNC_TIME"]);
                                }
                                if (!r.IsNull("SILENT_SYNC_TYPE"))
                                {
                                    pnw.SilentSyncType = (SilentSyncType)r["SILENT_SYNC_TYPE"];
                                }
                                if (!r.IsNull("AUTO_SYNC_RESTART"))
                                {
                                    pnw.AutomaticSyncRestart = (bool)r["AUTO_SYNC_RESTART"];
                                }
                            }
                            else
                            {
            ...
            try
            {
                using (var oData = new SQLiteDataObject(ConnectionString))
                {
                    foreach (var t in tags)
                    {
                        ...
                        sb.Append("')");
                        oData.Execute(sb.ToString());
                        PNCollections.Instance.Tags.Add(t);
                    }
                }
            }
            catch (Exception ex)
        ...
        internal static void SaveGridColumnVisibility(FixedWidthColumn column, string gridName)
        {
            try
            {
                ...
                sb.Append(" WHERE CN_KEY = '");
                sb.Append(WPFGridViewHelper.GetColumnName(column));
                sb.Append("'");
                ExecuteTransactionForStringBuilder(sb, SettingsConnectionString);
            }
            catch (Exception ex)
        ...
        internal static void SaveGridColumnsOrder(GridViewColumnCollection cols, string gridName)
        {
            try
            {
                ...
                for (var i = 0; i < cols.Count; i++)
                {
                    ...
                    sb.Append(i);
                    sb.Append(" WHERE CN_KEY = '");
                    sb.Append(WPFGridViewHelper.GetColumnName(cols[i]));
                    sb.Append("'");
                ...
                using (var oData = new SQLiteDataObject(SettingsConnectionString))
                {
                    if (oData.TableExists(tableName))
                    {
                        ...
                        using (var t = oData.FillDataTable("SELECT * FROM " + tableName))
                        {
                            ...
                            {
                                var column =
                                    grd.Columns.Where(t1 => WPFGridViewHelper.GetColumnName(t1) == (string)r["CN_KEY"])
                    ...
                    else
                    {
                        ...
                        for (var i = 0; i < grd.Columns.Count; i++)
                        {
                            ...
                            sb.Append("'");
                            sb.Append(WPFGridViewHelper.GetColumnName(c));
                            sb.Append("', ");
                            sb.Append(1);
                            sb.Append(", ");
                            sb.Append(i);
                            ...
                            cols.Add(new ColumnProps
                            {
                                Name = WPFGridViewHelper.GetColumnName(c),
        ...
        internal static void SaveLastPage()
        {
            try
            {
                using (var oData = new SQLiteDataObject(SettingsConnectionString))
                {
                    var sqlQuery = "UPDATE CONFIG SET LAST_PAGE = " + PNRuntimes.Instance.Settings.Config.LastPage.ToString(PNRuntimes.Instance.CultureInvariant);
                    oData.Execute(sqlQuery);
                }
            }
            catch (Exception ex)
                ...
                using (var oData = new SQLiteDataObject(SettingsConnectionString))
                {
                    var sqlQuery = !isDeafault
                                          ? "UPDATE CONFIG SET CONTROLS_STYLE = '" +
                                            PNRuntimes.Instance.Settings.Config.ControlsStyle +
                                            "'"
        ...
        internal static void DeleteGroups(List<int> ids)
        {
            try
            {
                ...
                foreach (var id in ids)
                {
                    ...
                    sb.Append(id.ToString(PNRuntimes.Instance.CultureInvariant) + "_show';");
                    sb.AppendLine();
                    sb.Append("DELETE FROM HOT_KEYS WHERE MENU_NAME = '");
                    sb.Append(id.ToString(PNRuntimes.Instance.CultureInvariant) + "_hide';");
                }
                if (ExecuteTransactionForStringBuilder(sb, ConnectionString))
                {
                    foreach (var id in ids)
                    {
                        var prefix = id.ToString(PNRuntimes.Instance.CultureInvariant) + "_";
                        PNCollections.Instance.HotKeysGroups.RemoveAll(hk => hk.MenuName.StartsWith(prefix));
                    }
                }
            }
            catch (Exception ex)
        ...
        internal static void SaveGroupChanges(PNGroup group)
        {
            try
            {
                ...
                sb.Append(c.ConvertToString(null, PNRuntimes.Instance.CultureInvariant, group.Skinless.BackColor));
                sb.Append("', CAPTION_FONT_COLOR = '");
                sb.Append(c.ConvertToString(null, PNRuntimes.Instance.CultureInvariant, group.Skinless.CaptionColor));
                ...
                sb.Append(dcc.ConvertToString(null, PNRuntimes.Instance.CultureInvariant, group.FontColor));
                sb.Append("', FONT = '");
        ...
        internal static bool IsMediaColor(string colorString)
        {
            try
            {
                ...
                mediaColorConverter.ConvertFromString(null, PNRuntimes.Instance.CultureInvariant, colorString);
                return true;
            }
            ...
            try
            {
                            ...
                            "SELECT GROUP_ID, CAPTION_FONT FROM GROUPS WHERE CAPTION_FONT LIKE '%^%'"))
                {
                    foreach (DataRow r in t.Rows)
                    {
                        ...
                        sb.Append(wfc.ConvertToString(null, PNRuntimes.Instance.CultureInvariant, fnt));
                        sb.Append("' WHERE GROUP_ID = ");
                        sb.Append(r["GROUP_ID"]);
                        listSql.Add(sb.ToString());
                    }
                ...
                using (var t = oData.FillDataTable("SELECT GROUP_ID, BACK_COLOR, CAPTION_FONT_COLOR FROM GROUPS"))
                {
                    ...
                    {
                        var convertFromString = drawingColorConverter.ConvertFromString(null, PNRuntimes.Instance.CultureInvariant, cr.Color);
                        ...
                        sb.Append(mediaColorConverter.ConvertToString(null, PNRuntimes.Instance.CultureInvariant, color));
                        sb.Append("' WHERE GROUP_ID = ");
                        sb.Append(cr.Id);
                        listSql.Add(sb.ToString());
                    }
        ...
        internal static void InsertNewGroup(PNGroup group)
        {
            try
            {
                ...
                sb.Append(lfc.ConvertToString(group.Font));
                sb.Append("','");
                sb.Append(drcc.ConvertToString(null, PNRuntimes.Instance.CultureInvariant, group.FontColor));
                sb.Append("', ");
                sb.Append(Convert.ToInt32(group.IsDefaultImage));
                sb.Append("); ");
                if (ExecuteTransactionForStringBuilder(sb, ConnectionString))
                {
                    ...
                    sb.Append(((int)HotkeyType.Group).ToString(PNRuntimes.Instance.CultureInvariant));
                    sb.Append(",'");
                    sb.Append(prefix);
                    sb.Append("',");
                    sb.Append(id.ToString(PNRuntimes.Instance.CultureInvariant));
                    sb.Append(",'');");
                    if (ExecuteTransactionForStringBuilder(sb, ConnectionString))
                    {
                        PNCollections.Instance.HotKeysGroups.Add(new PNHotKey { MenuName = prefix, Id = id, Type = HotkeyType.Group });
                    }
                    sb = new StringBuilder();
                    id++;
        ...
        internal static void SaveSearchNotesSettings()
        {
            try
            {
                var pcf = PNRuntimes.Instance.Settings.Config;
        ...
        internal static void SaveLastSync(bool local)
        {
            try
            {
                var sb = new StringBuilder();
                ...
                if (local)
                {
                    sb.Append("UPDATE CONFIG SET LAST_LOCAL_SYNC = '");
                    sb.Append(pcf.LastLocalSync.ToString("dd MMM yyyy", PNRuntimes.Instance.CultureInvariant));
                    sb.Append("'");
                }
                else
                {
                    sb.Append("UPDATE CONFIG SET LAST_PLUGIN_SYNC = '");
                    sb.Append(pcf.LastPluginSync.ToString("dd MMM yyyy", PNRuntimes.Instance.CultureInvariant));
                    sb.Append("'");
                }
                ExecuteTransactionForStringBuilder(sb, SettingsConnectionString);
            }
        ...
        internal static void SaveCPProperties()
        {
            try
            {
                ...
                sb.Append(crc.ConvertToString(null, PNRuntimes.Instance.CultureInvariant, pcf.CPPvwColor));
                sb.Append("',CP_USE_CUST_PVW_COLOR = ");
                sb.Append(Convert.ToInt32(pcf.CPUseCustPvwColor));
                sb.Append(",CP_SIZE = '");
                sb.Append(sc.ConvertToString(null, PNRuntimes.Instance.CultureInvariant, pcf.CPSize));
                sb.Append("',CP_LOCATION = '");
                sb.Append(pc.ConvertToString(null, PNRuntimes.Instance.CultureInvariant, pcf.CPLocation));
                ...
                sb.Append(",CP_IGNORE_CASE = ");
                sb.Append(Convert.ToInt32(pcf.CPIgnoreCase));
                ExecuteTransactionForStringBuilder(sb, SettingsConnectionString);
            }
            catch (Exception ex)
            {
        ...
        internal static void SaveProtectionSettings()
        {
            try
            {
                ...
                {
                    sb.Append("'");
                    sb.Append(pnp.FullBackupTime.ToString("HH:mm", PNRuntimes.Instance.CultureInvariant));
                    sb.Append("'");
                }
                else
                {
                ...
                {
                    sb.Append("'");
                    sb.Append(pnp.FullBackupDate.ToString("dd MMM yyyy HH:mm", PNRuntimes.Instance.CultureInvariant));
                    sb.Append("'");
                }
                else
                {
                ...
                sb.Append(", LOCAL_SYNC_FILES = '");
                sb.Append(pnp.LocalSyncFilesLocation);
                sb.Append("', LOCAL_SYNC_PERIOD = ");
                sb.Append(pnp.LocalSyncPeriod);
                sb.Append(", LOCAL_SYNC_TIME = '");
                sb.Append(new DateTime().Add(pnp.LocalSyncTime).ToString(PNRuntimes.Instance.CultureInvariant.DateTimeFormat.ShortTimePattern));
                sb.Append("', LOCAL_SYNC_TYPE = ");
                sb.Append(Convert.ToInt32(pnp.LocalSyncType));
                sb.Append(", AUTO_SYNC_RESTART = ");
                sb.Append(Convert.ToInt32(pnp.AutomaticSyncRestart));
        ...
        internal static void SaveNetworkSettings()
        {
            try
            {
                ...
                sb.Append(", STORE_ON_SERVER = ");
                sb.Append(Convert.ToInt32(pnw.StoreOnServer));
                sb.Append(", SERVER_IP = '");
                sb.Append(pnw.ServerIp);
                sb.Append("', SERVER_PORT = ");
                sb.Append(pnw.ServerPort);
                sb.Append(", SEND_TIMEOUT = ");
                sb.Append(pnw.SendTimeout);
                sb.Append(", SILENT_SYNC_PERIOD = ");
                sb.Append(pnw.SilentSyncPeriod);
                sb.Append(", SILENT_SYNC_TIME = '");
                sb.Append(new DateTime().Add(pnw.SilentSyncTime).ToString(PNRuntimes.Instance.CultureInvariant.DateTimeFormat.ShortTimePattern));
                sb.Append("', SILENT_SYNC_TYPE = ");
                sb.Append(Convert.ToInt32(pnw.SilentSyncType));
                sb.Append(", AUTO_SYNC_RESTART = ");
                sb.Append(Convert.ToInt32(pnw.AutomaticSyncRestart));
        ...
        internal static void SaveBehaviorSettings()
        {
            try
            {
                ...
                sb.Append(", OPACITY = ");
                sb.Append(pnb.Opacity.ToString(PNRuntimes.Instance.CultureInvariant));
        ...
        internal static void SaveScheduleSettings()
        {
            try
            {
                ...
                sb.Append(Convert.ToInt32(pnsc.FirstDayOfWeek));
                sb.Append(", FIRST_DAY_TYPE = ");
                sb.Append(Convert.ToInt32(pnsc.FirstDayOfWeekType));
                sb.Append(", STOP_AFTER = ");
                sb.Append(pnsc.StopAfter);
        ...
        internal static void SaveHiddenMenus()
        {
            try
            {
                ...
                foreach (var m in PNCollections.Instance.HiddenMenus)
                {
                    var sb = new StringBuilder();
        ...
        internal static void UpdateContact(PNContact cn)
        {
            try
            {
                var sb = new StringBuilder($"UPDATE {PNStrings.T_CONTACTS} SET GROUP_ID = ");
                ...
                sb.Append(" WHERE ID = ");
                sb.Append(cn.Id);
                ExecuteTransactionForStringBuilder(sb, ContactsConnectionString);
            }
            catch (Exception ex)
            {
                PNStatic.LogException(ex);
        ...
        internal static void SaveContacts()
        {
            try
            {
                var sqlList = new List<string> { $"DELETE FROM {PNStrings.T_CONTACTS}" };
                foreach (var cn in PNCollections.Instance.Contacts)
                {
                    var sb = new StringBuilder();
                    sb.Append($"INSERT INTO {PNStrings.T_CONTACTS} VALUES(");
                    sb.Append(cn.Id);
                    sb.Append(",");
                    sb.Append(cn.GroupId);
                    sb.Append(",'");
                ...
                ExecuteTransactionForList(sqlList, ContactsConnectionString);
            }
            catch (Exception ex)
            {
                PNStatic.LogException(ex);
        ...
        internal static void SaveSocialPlugins()
        {
            try
            {
                ...
                foreach (var p in PNCollections.Instance.ActivePostPlugins)
                {
                    var sb = new StringBuilder();
        ...
        internal static void SaveSyncPlugins()
        {
            try
            {
                ...
                foreach (var p in PNCollections.Instance.ActiveSyncPlugins)
                {
                    var sb = new StringBuilder();
        ...
        internal static void SaveTags()
        {
            try
            {
                ...
                foreach (var s in PNCollections.Instance.Tags)
                {
                    var sb = new StringBuilder();
        ...
        internal static void SaveMailContacts()
        {
            try
            {
                var sqlList = new List<string> { $"DELETE FROM {PNStrings.T_MAIL_CONTACTS}" };
                foreach (var c in PNCollections.Instance.MailContacts)
                {
                    var sb = new StringBuilder($"INSERT INTO {PNStrings.T_MAIL_CONTACTS} VALUES(");
                    sb.Append(c.Id);
                    sb.Append(", '");
        ...
        internal static void SaveSmtpClients()
        {
            try
            {
                ...
                foreach (var c in PNCollections.Instance.SmtpProfiles)
                {
        ...
        internal static void SaveSearchProviders()
        {
            try
            {
                ...
                foreach (var s in PNCollections.Instance.SearchProviders)
                {
                    var sb = new StringBuilder();
        ...
        internal static void SaveExternals()
        {
            try
            {
                ...
                foreach (var e in PNCollections.Instance.Externals)
                {
                    var sb = new StringBuilder();
        ...
        internal static void SaveSyncComps()
        {
            try
            {
                ...
                foreach (var sc in PNCollections.Instance.SyncComps)
                {
                    var sb = new StringBuilder();
        ...
        internal static void SaveContactGroups()
        {
            try
            {
                var sqlList = new List<string> { $"DELETE FROM {PNStrings.T_CONTACT_GROUPS}" };
                foreach (var cg in PNCollections.Instance.ContactGroups)
                {
                    var sb = new StringBuilder();
                    sb.Append($"INSERT INTO {PNStrings.T_CONTACT_GROUPS} VALUES(");
                    sb.Append(cg.Id);
                    sb.Append(",'");
        ...
        internal static void SaveGeneralSettings()
        {
            try
            {
                ...
                sb.Append(cc.ConvertToString(null, PNRuntimes.Instance.CultureInvariant, pngeneral.SpellColor));
                sb.Append("', USE_SKINS = ");
        ...
        private static void recreateGeneralSettingsTable()
        {
            try
            {
                var sqlQuery =
                    new StringBuilder(
                        "CREATE TABLE [GENERAL_SETTINGS_TEMP] ([LANGUAGE] TEXT, [RUN_ON_START] BOOLEAN, [SHOW_CP_ON_START] BOOLEAN, [CHECK_NEW_VERSION_ON_START] BOOLEAN, [HIDE_TOOLBAR] BOOLEAN, [USE_CUSTOM_FONTS] BOOLEAN, [SHOW_SCROLLBAR] INT, [HIDE_DELETE_BUTTON] BOOLEAN, [CHANGE_HIDE_TO_DELETE] BOOLEAN, [HIDE_HIDE_BUTTON] BOOLEAN, [BULLETS_INDENT] INT, [MARGIN_WIDTH] INT, [SAVE_ON_EXIT] BOOLEAN, [CONFIRM_SAVING] BOOLEAN, [CONFIRM_BEFORE_DELETION] BOOLEAN, [SAVE_WITHOUT_CONFIRM_ON_HIDE] BOOLEAN, [WARN_ON_AUTOMATICAL_DELETE] BOOLEAN, [AUTO_SAVE] BOOLEAN, [AUTO_SAVE_PERIOD] INT, [REMOVE_FROM_BIN_PERIOD] INT, [DATE_FORMAT] TEXT, [TIME_FORMAT] TEXT, [SKINLESS_WIDTH] INT, [SKINLESS_HEIGHT] INT, [SPELL_COLOR] TEXT, [USE_SKINS] BOOLEAN, [SPELL_MODE] INT, [SPELL_DICT] TEXT, [DOCK_WIDTH] INT, [DOCK_HEIGHT] INT, [SHOW_PRIORITY_ON_START] BOOLEAN, [BUTTONS_SIZE] INT, [AUTOMATIC_SMILIES] BOOLEAN, [SPACE_POINTS] INT, [RESTORE_AUTO] BOOLEAN, [PARAGRAPH_INDENT] INT, [AUTO_HEIGHT] BOOLEAN, [CRITICAL_ON_START] BOOLEAN, [CRITICAL_PERIODICALLY] BOOLEAN, [DELETE_SHORTCUTS_ON_EXIT] BOOLEAN, [RESTORE_SHORTCUTS_ON_START] BOOLEAN, [CLOSE_ON_SHORTCUT] BOOLEAN); ");
                sqlQuery.Append("INSERT INTO GENERAL_SETTINGS_TEMP SELECT * FROM GENERAL_SETTINGS; ");
                sqlQuery.Append("DROP TABLE GENERAL_SETTINGS; ");
                sqlQuery.Append("ALTER TABLE GENERAL_SETTINGS_TEMP RENAME TO GENERAL_SETTINGS");
                ExecuteTransactionForStringBuilder(sqlQuery, SettingsConnectionString);
            }
            catch (Exception ex)
            {
                PNStatic.LogException(ex);
