...
namespace PNotes.NET
{
    internal class PNSync
    {
        ...
        //todo
        private bool exchangeNotes(SQLiteDataObject dataSrc, SQLiteDataObject dataDest, string id, KeyValuePair<string, List<FieldData>> tableData)
        {
            try
            {
        ...
        private bool exchangeCustomNotesSettings(SQLiteDataObject dataSrc, SQLiteDataObject dataDest, string id, KeyValuePair<string, List<FieldData>> tableData)
        {
            try
            {
        ...
        private bool exchangeGroups(SQLiteDataObject dataSrc, SQLiteDataObject dataDest, KeyValuePair<string, List<FieldData>> tableData)
        {
            try
            {
        ...
        private bool exchangeNotesSchedule(SQLiteDataObject dataSrc, SQLiteDataObject dataDest, string id, KeyValuePair<string, List<FieldData>> tableData)
        {
            try
            {
                using (var t1 = dataSrc.FillDataTable($"SELECT * FROM NOTES_SCHEDULE WHERE NOTE_ID = '{id}' AND SCHEDULE_TYPE <> 0"))
                {
                    using (var t2 = dataDest.FillDataTable($"SELECT * FROM NOTES_SCHEDULE WHERE NOTE_ID = '{id}' AND SCHEDULE_TYPE <> 0"))
                    {
                        if (t1.Rows.Count > 0 && t2.Rows.Count > 0)
                        {
                            var d1 = Convert.ToInt64(t1.Rows[0]["UPD_DATE"]);
        ...
        private bool exchangeLinkedNotes(SQLiteDataObject dataSrc, SQLiteDataObject dataDest, string id, KeyValuePair<string, List<FieldData>> tableData)
        {
            try
            {
        ...
        private bool exchangeNotesTags(SQLiteDataObject dataSrc, SQLiteDataObject dataDest, string id, KeyValuePair<string, List<FieldData>> tableData)
        {
            try
            {
        ...
        //todo
        private bool exchangeData(SQLiteDataObject dataSrc, SQLiteDataObject dataDest, string id, Dictionary<string, List<FieldData>> tablesData)
        {
            try
            {
                //todo
        ...
        //private bool dropTriggers(string dbPath)
        //{
        //    try
        //    {
        ...
        //private bool restoreTriggers(string dbPath)
        //{
        //    try
        //    {
        //        var connSrc = "data source = " + dbPath;
        ...
        internal void SyncLocal(string path)
        {
            var result = LocalSyncResult.Reload;
            var filesSrc = new List<NoteFile>();
            var filesDest = new List<NoteFile>();
            var tempDir = "";

            try
            {
                if (PNRuntimes.Instance.Settings.Network.SaveBeforeSync)
                {
                ...
                var destDir = path;
                var destDB = Path.Combine(destDir, PNStrings.DB_FILE);
                ...
                using (var eSrc = new SQLiteDataObject(srcConnectionString))
                {
                    ...
                    filesSrc.AddRange(srcNotes.Select(fi => new NoteFile { Path = fi.FullName, Name = fi.Name, Copy = false }));
                    // get deleted ids
                    if (!PNRuntimes.Instance.Settings.Network.IncludeBinInSync)
                    {
                    ...
                    filesDest.AddRange(destNotes.Select(fir => new NoteFile { Path = fir.FullName, Name = fir.Name, Copy = false }));
                    ...
                    if (File.Exists(destDB))
                    {
                        ...
                        using (var eDest = new SQLiteDataObject(destConnectionString))
                        {
                            ...
                            {
                                SyncComplete?.Invoke(this, new LocalSyncCompleteEventArgs(LocalSyncResult.AbortVersion));
                                return;
                            }

                            //todo: create tables data
                            var tablesData = new Dictionary<string, List<FieldData>>();
                            foreach (var tn in _Tables)
                            {
                                var td = new List<FieldData>();
                            ...
                            foreach (var sf in filesSrc)
                            {
                                ...
                                if (df == null)
                                {
                                    sf.Copy = true;
                                    //todo
                                    if (insertToAllTables(eSrc, eDest, id, tablesData)) continue;
                                    SyncComplete?.Invoke(this, new LocalSyncCompleteEventArgs(LocalSyncResult.Error));
                                    return;
                                }
                                else
                                {
                                    ...
                                    //todo
                                    if (exchangeData(eSrc, eDest, id, tablesData)) continue;
                                    SyncComplete?.Invoke(this, new LocalSyncCompleteEventArgs(LocalSyncResult.Error));
                                    return;
                                }
                            }
                            ...
                            {
                                if (filesSrc.Any(sf => sf.Name == df.Name)) continue;
                                df.Copy = true;
                ...
                SyncComplete?.Invoke(this, new LocalSyncCompleteEventArgs(result));
            }
            catch (Exception ex)
            {
                PNStatic.LogException(ex);
        ...
        private List<string> deletedIDs(SQLiteDataObject oData)
        {
            ...
            try
            {
                using (var t = oData.FillDataTable($"SELECT ID FROM NOTES WHERE GROUP_ID = {((int)SpecialGroups.RecycleBin).ToString()}"))
                {
        ...
        private string createInsert(DataRow r, KeyValuePair<string, List<FieldData>> tableData)
        {
            try
            {
                    ...
                    if (f.NotNull)
                    {
                        ...
                        switch (f.Type)
                        {
                            ...
                            case "TEXT":
                                ...
                                break;
                            case "REAL":
                                sb.Append(Convert.ToString(r[f.Name], PNRuntimes.Instance.CultureInvariant));
                                break;
                            default:
                                sb.Append(r[f.Name]);
                                break;
        ...
        private bool insertToNotesTags(SQLiteDataObject dataDest, DataRow r, KeyValuePair<string, List<FieldData>> tableData)
        {
            try
            {
                var sb = new StringBuilder();
        ...
        //todo
        private bool insertToNotes(SQLiteDataObject dataDest, DataRow r, KeyValuePair<string, List<FieldData>> tableData)
        {
            try
            {
        ...
        private bool insertToCustomNotesSettings(SQLiteDataObject dataDest, DataRow r, KeyValuePair<string, List<FieldData>> tableData)
        {
            try
            {
                var sb = new StringBuilder();
        ...
        private bool insertToLinkedNotes(SQLiteDataObject dataDest, DataRow r, KeyValuePair<string, List<FieldData>> tableData)
        {
            try
            {
                var sb = new StringBuilder();
        ...
        //todo: table data
        private bool insertToAllTables(SQLiteDataObject dataSrc, SQLiteDataObject dataDest, string id, Dictionary<string, List<FieldData>> tablesData)
        {
            try
            {
                ...
                using (var t = dataSrc.FillDataTable($"SELECT * FROM NOTES_SCHEDULE WHERE NOTE_ID = '{id}' AND SCHEDULE_TYPE <> 0"))
                {
                    if (t.Rows.Count > 0)
                    {
        ...
        private bool insertToNotesSchedule(SQLiteDataObject dataDest, DataRow r, KeyValuePair<string, List<FieldData>> tableData)
        {
            try
            {
                var sb = new StringBuilder();
