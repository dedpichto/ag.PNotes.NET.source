...
namespace PNotes.NET
{
    ...
    public partial class WndMenusManager
    {
        ...
        private void DlgMenusManager_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                PNWindows.Instance.FormMenus = this;
        ...
        private void resetCurrentClick()
        {
            try
            {
                switch (tabMenus.SelectedIndex)
        ...
        private void moveUpDown(UpDown direction)
        {
            try
            {
                ...
                if (!(tvw?.SelectedItem is PNTreeItem item)) return;
                var parent = item.Parent;
                var index = item.Index;
                var parentItem = parent as PNTreeItem;
                var items = parentItem?.Items ?? tvw.Items;
            ...
            try
            {
                ...
                var treeItem = mnu.Text != PNStrings.MENU_SEPARATOR_STRING
                    ? new PNTreeItem("mnu", mnu.Text, mnu, mnu.Name,
                        PNCollections.Instance.HiddenMenus.Where(m => m.Type == type).All(m => m.Name != mnu.Name))
                    {
                        IsExpanded = true
                    }
                    : new PNTreeItem("mnu", mnu.Text, mnu, mnu.Name);
        ...
        private void createOrderSql(PNTreeItem node)
        {
            try
            {
                ...
                if (!(node.Tag is PNMenu pnm)) return;
        ...
        private bool saveMenus()
        {
            try
            {
                ...
                PNCollections.Instance.HiddenMenus.AddRange(_TempHiddenMenus);

                PNData.SaveHiddenMenus();
                PNWindows.Instance.FormMain.RefreshHiddenMenus();
                if (_OrderSql.Count > 0)
                {
                    if (PNData.ExecuteTransactionForList(_OrderSql, PNData.ConnectionString))
                    {
                        PNWindows.Instance.FormMain.ApplyNewMenusOrder(_MenusOrderChanged);
                    }
                }
                return true;
            }
        ...
        private void treeItem_TreeViewItemCheckChanged(object sender, TreeViewItemCheckChangedEventArgs e)
        {
            try
            {
                if (!(sender is PNTreeItem item) || e.ParentTreeView == null) return;
        ...
        {
            PNWindows.Instance.FormMenus = null;
        }
        ...
        private void CommandBinding_CanExecute(object sender, System.Windows.Input.CanExecuteRoutedEventArgs e)
        {
            try
            {
                ...
                switch (command.Type)
                {
                    ...
                    case CommandType.ResetCurrent:
                        e.CanExecute = true;
                        break;
                    case CommandType.RestoreOrder:
                    case CommandType.Up:
                    case CommandType.Down:
                        ...
                        if (command.Type == CommandType.RestoreOrder)
                        {
                            e.CanExecute = item != null;
                        }
                        else if (command.Type == CommandType.Up)
                        {
                            if (item == null)
                                e.CanExecute = false;
                            else
                                e.CanExecute = item.Index > 0;
                        }
                        else
                        {
                            if (item == null)
                            ...
                            else
                            {
                                ...
                                e.CanExecute = item.Index < parent.Items.Count - 1;
                            }
                        }
                        break;
                }
