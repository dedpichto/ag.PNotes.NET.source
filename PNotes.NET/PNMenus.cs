...
namespace PNotes.NET
{
    internal class PNMenus
    {
        ...
        internal static void SetEnabledByHiddenMenus(ItemsControl parent)
        {
            try
            {
                parent.IsEnabled = !parent.Items.OfType<MenuItem>().All(mi =>
                    mi.Visibility == System.Windows.Visibility.Collapsed && parent.Items.OfType<Separator>()
                        .All(sp => sp.Visibility == System.Windows.Visibility.Collapsed));
                foreach (var item in parent.Items.OfType<MenuItem>())
                {
                    if (item.Visibility == System.Windows.Visibility.Visible && item.HasItems)
                    {
                        if (item.Items.OfType<MenuItem>().All(mi =>
                            mi.Visibility == System.Windows.Visibility.Collapsed && item.Items.OfType<Separator>()
                                .All(sp => sp.Visibility == System.Windows.Visibility.Collapsed)))
                            item.IsEnabled = false;
                        else
                        {
                            item.IsEnabled = true;
                            SetEnabledByHiddenMenus(item);
