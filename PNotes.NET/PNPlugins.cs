...
namespace PNotes.NET
{
    internal sealed class PNPlugins
    {
        ...
        internal static PNPlugins Instance => LazyF.Value;
        ...
        internal List<IPostPlugin> SocialPlugins => Instance._PostPlugins;

        internal List<ISyncPlugin> SyncPlugins => Instance._SyncPlugins;
        ...
        internal void LoadPlugins(IPluginsHost host, string pluginsDir)
        {
            ...
            foreach (var type in plugins.Factory.GetPlugins().Select(p => p.GetType()))
            {
                if (type.GetInterface("PluginsCore.ISyncPlugin") != null)
                    Instance._SyncPlugins.Add((ISyncPlugin)Activator.CreateInstance(type));
                else if (type.GetInterface("PluginsCore.IPostPlugin") != null)
                    Instance._PostPlugins.Add((IPostPlugin)Activator.CreateInstance(type));
            }
            ...
            //}
            foreach (var p in Instance._SyncPlugins)
            {
                p.Init(host);
                p.BeforeSync += sync_BeforeSync;
                p.SyncComplete += sync_SyncComplete;
            }
            foreach (var p in Instance._PostPlugins)
            {
                p.Init(host);
        ...
        internal static string GetPluginDirectory(string pluginName, string pluginsDir)
        {
            try
            {
                ...
                ////replace 'onedrive' back to 'skydrive'
                //pluginName = pluginName.Replace("onedrive", "skydrive");
        ...
        private void sync_SyncComplete(object sender, SyncCompleteEventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (sender is ISyncPlugin plugin)
                    {
                        caption = plugin.Name;
                    }
                    ...
                    switch (e.Result)
                    {
                        ...
                        case SyncResult.Error:
                            ...
                            PNWindows.Instance.FormMain.ntfPN.ShowCustomBalloon(baloon, PopupAnimation.Slide, null);
                            break;
                    }
                }
            }
        ...
        {
            if (!(sender is IPostPlugin p)) return;
            if (!e.Result) return;
            var message =
