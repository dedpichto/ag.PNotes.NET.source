...
namespace PNotes.NET
{
    internal class EditControl
    {
        ...
        public EditControl(FrameworkElement placementTarget)
        {
            ...
            if (owner.Background is SolidColorBrush brush)
            {
            ...
            var dp = DependencyPropertyDescriptor.FromProperty(FrameworkElement.FlowDirectionProperty, typeof(Window));
            dp?.AddValueChanged(owner, (sender, args) =>
            {
                owner.Width += 1;
                owner.Width -= 1;
            });
        ...
        {
            var d = new RepositionDelegate(Reposition);
            //begin invoke reposition
            if (_repositionCallback == null)
                _repositionCallback = _WpfWindow.Dispatcher.BeginInvoke(d);
        }

        private delegate void RepositionDelegate();

        private void Reposition()
        {
            //release DispatcherOperation
            _repositionCallback = null;
            if (!_WinForm.Visible) return;
            //get WPF Window location
            var translatePoint = _PlacementTarget.TranslatePoint(new Point(), _WpfWindow);

            if (_WpfWindow.FlowDirection == System.Windows.FlowDirection.RightToLeft)
            {
                var pts = new[]
                {
                    new Point(translatePoint.X, translatePoint.Y),
                    new Point(translatePoint.X - _PlacementTarget.ActualWidth, translatePoint.Y)
                };
                var m = Matrix.Identity;
                m.RotateAt(180, _WpfWindow.ActualWidth / 2, 0);
                m.Transform(pts);
                translatePoint.X = pts[1].X;
            }
            ...
            EditControlSizeChanged?.Invoke(this,
