...
namespace PNotes.NET
{
    ...
    internal static class HotkeysStatic
    {
        ...
        {
            var keys = Keys.None;
            if ((hk.Modifiers & HotkeyModifiers.ModControl) > 0)
                keys |= Keys.Control;
            if ((hk.Modifiers & HotkeyModifiers.ModAlt) > 0)
                keys |= Keys.Menu;
            if ((hk.Modifiers & HotkeyModifiers.ModShift) > 0)
                keys |= Keys.Shift;
            if ((hk.Modifiers & HotkeyModifiers.ModWin) > 0)
                keys |= Keys.LWin;
            switch (hk.Vk)
            {
                case VK_0:
        ...
        {
            var result = HotkeyModifiers.ModNone;
            if ((GetKeyState(VK_CONTROL) & 0x8000) == 0x8000)
            {
                result |= HotkeyModifiers.ModControl;
                text += "Ctrl+";
            }
            if ((GetKeyState(VK_SHIFT) & 0x8000) == 0x8000)
            {
                result |= HotkeyModifiers.ModShift;
                text += "Shift+";
            }
            if ((GetKeyState(VK_MENU) & 0x8000) == 0x8000)
            {
                result |= HotkeyModifiers.ModAlt;
                text += "Alt+";
            }
            if ((GetKeyState(VK_LWIN) & 0x8000) == 0x8000 || (GetKeyState(VK_RWIN) & 0x8000) == 0x8000)
            {
                result |= HotkeyModifiers.ModWin;
                text += "Win+";
            }
            return result;
        }
