...
namespace PNotes.NET
{
    ...
    public partial class WndPanel
    {
        ...
        internal void UpdateAutoHideImageBinding()
        {
            try
            {
                if (!(imgAutoHide.RenderTransform is RotateTransform tr)) return;
        ...
        internal void UpdateOrientationImageBinding()
        {
            try
            {
                if (!(imgOrientation.RenderTransform is RotateTransform tr)) return;
                var bindingExpression = BindingOperations.GetBindingExpression(tr, RotateTransform.AngleProperty);
                bindingExpression?.UpdateTarget();
            ...
            try
            {
                switch (PNRuntimes.Instance.Settings.Behavior.NotesPanelOrientation)
                {
                    case NotesPanelOrientation.Left:
                        ...
                        Left = PNRuntimes.Instance.Settings.Behavior.PanelAutoHide ? -SIZE_VERT + 2 : 0;
                    ...
                    case NotesPanelOrientation.Top:
                        ...
                        Top = PNRuntimes.Instance.Settings.Behavior.PanelAutoHide ? -SIZE_HORZ + 2 : 0;
        ...
        private void removeThumbnail(ThumbnailButton thb)
        {
            try
            {
                var note = PNCollections.Instance.Notes.FirstOrDefault(n => n.Id == thb.Id);
                thb.Click -= Thumbnail_Click;
                thb.MouseEnter -= Thumbnail_MouseEnter;
                thb.MouseLeave -= Thumbnail_MouseLeave;
                thb.MouseDoubleClick -= Thumbnail_MouseDoubleClick;
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                var coefficient = TryFindResource("ThumbFullGrow");
                _GrowSizeCoefficient = coefficient != null ? Convert.ToDouble(coefficient) : 2.0;
                SetPanelPlacement();
        ...
        void Thumbnail_MouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            try
            {
                if (PNRuntimes.Instance.Settings.Behavior.PanelRemoveMode != PanelRemoveMode.DoubleClick) return;
                if (!(sender is ThumbnailButton thb)) return;
                removeThumbnail(thb);
        ...
        private void Thumbnail_MouseLeave(object sender, MouseEventArgs e)
        {
            try
            {
                if (PNRuntimes.Instance.Settings.Behavior.PanelSwitchOffAnimation) return;
                if (!(sender is ThumbnailButton button)) return;
                if (!(TryFindResource("NormalSizeGrow") is Storyboard stb)) return;
        ...
        private void Thumbnail_MouseEnter(object sender, MouseEventArgs e)
        {
            try
            {
                if (PNRuntimes.Instance.Settings.Behavior.PanelSwitchOffAnimation) return;
                if (!(sender is ThumbnailButton button)) return;
                if (!(TryFindResource("HalfsSizeGrow") is Storyboard stb)) return;
                button.Prev?.BeginStoryboard(stb);
                button.Next?.BeginStoryboard(stb);
            }
            catch (Exception ex)
        ...
        private void Thumbnail_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (PNRuntimes.Instance.Settings.Behavior.PanelRemoveMode != PanelRemoveMode.SingleClick) return;
                if (!(sender is ThumbnailButton thb)) return;
                removeThumbnail(thb);
                allButtonsToNormalSize();
            }
        ...
        private void Window_MouseEnter(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                if (!PNRuntimes.Instance.Settings.Behavior.PanelAutoHide) return;
                if (!_Hidden) return;
                if (PNRuntimes.Instance.Settings.Behavior.PanelEnterDelay == 0)
                {
                    mouseEntered();
                }
                else
                {
                    _DelayTimer.Interval = PNRuntimes.Instance.Settings.Behavior.PanelEnterDelay;
                    _DelayTimer.Start();
                }
            }
            catch (Exception ex)
        ...
        private bool hidePanel()
        {
            try
            {
                ...
                if (!PNRuntimes.Instance.Settings.Behavior.PanelAutoHide) return false;
        ...
        private void Pin_MouseLeftButtonDown(object sender, MouseButtonEventArgs e)
        {
            try
            {
                PNRuntimes.Instance.Settings.Behavior.PanelAutoHide = !PNRuntimes.Instance.Settings.Behavior.PanelAutoHide;
                PNData.SaveBehaviorSettings();
                UpdateAutoHideImageBinding();
                if (PNWindows.Instance.FormSettings != null)
                {
                    PNWindows.Instance.FormSettings.PanelAutohideChanged();
                }
                if (PNRuntimes.Instance.Settings.Behavior.PanelAutoHide)
                {
                    _PreventMove = true;
                    switch (PNRuntimes.Instance.Settings.Behavior.NotesPanelOrientation)
                    {
        ...
        private void Orient_MouseLeftButtonDown(object sender, MouseButtonEventArgs e)
        {
            try
            {
                PNRuntimes.Instance.Settings.Behavior.NotesPanelOrientation = PNRuntimes.Instance.Settings.Behavior.NotesPanelOrientation ==
                ...
                PNWindows.Instance.FormSettings.PanelOrientationChanged();
            }
            catch (Exception ex)
            {
                PNStatic.LogException(ex);
