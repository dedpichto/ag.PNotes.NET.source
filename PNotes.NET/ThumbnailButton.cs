...
namespace PNotes.NET
{
    ...
    public class ThumbnailButton : Button
    {
        ...
        {
            get => (string) GetValue(ThumbnailNameProperty);
            set => SetValue(ThumbnailNameProperty, value);
        }

        public Brush ThumbnailBrush
        {
            get => (Brush) GetValue(ThumbnailBrushProperty);
        ...
        {
            if (!PNRuntimes.Instance.Settings.Behavior.PanelSwitchOffAnimation)
            {
                if (TryFindResource("ThumbnailEnter") is Storyboard sb)
                {
                    BeginStoryboard(sb);
                }
            }
        ...
        protected override void OnMouseLeave(System.Windows.Input.MouseEventArgs e)
        {
            if (!PNRuntimes.Instance.Settings.Behavior.PanelSwitchOffAnimation)
            {
                if (TryFindResource("ThumbnailLeave") is Storyboard sb)
                {
                    BeginStoryboard(sb);
                }
            }
