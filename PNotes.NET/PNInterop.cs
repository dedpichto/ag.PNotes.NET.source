...
namespace PNotes.NET
{
    internal class PNInterop
    {
        ...
        internal struct WindowPlacement
        {
            ...
            internal PointLong ptMinPosition;
            internal PointLong ptMaxPosition;
            internal RectLong rcNormalPosition;
        }
            ...
            try
            {
                ...
                foreach (FileInfo f in fonts)
                {
                    if (exts.Contains(f.Extension))
                    {
                        ...
                        {
                            PNCollections.Instance.CustomFonts.Add(f.FullName);
                        }
                    }
                }
            }
        ...
        internal static void RemoveCustomFonts()
        {
            try
            {
                foreach (string f in PNCollections.Instance.CustomFonts)
                {
        ...
        internal static List<string> GetNetworkComputers()
        {
            ...
            var sizeofInfo = Marshal.SizeOf(typeof(ServerInfo100));
            ...
            try
            {
                ...
                var ret = NetServerEnum(null, 100, ref buffer, MAX_PREFERRED_LENGTH, out _, out var totalEntries,
                    SV_TYPE_WORKSTATION | SV_TYPE_SERVER, null, out _);
                //if the returned with a NERR_Success 
                //(C++ term), =0 for C#
                if (ret != 0) return networkComputers;
